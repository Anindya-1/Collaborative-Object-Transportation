...
using Trajectory2DGoalHandle = rclcpp_action::ClientGoalHandle<Trajectory2D>;
...
private:
    ...
    void send_leader_goal(std::vector<geometry_msgs::msg::Vector3> trajectory);
    void send_follower_goal(std::vector<geometry_msgs::msg::Vector3> trajectory);

    // ROS2 action client
    rclcpp_action::Client<Trajectory2D>::SharedPtr leader_trajectory_client_;
    rclcpp_action::Client<Trajectory2D>::SharedPtr follower_trajectory_client_;

    // ROS2 action callback functions
    void goal_result_callback(const Trajectory2DGoalHandle::WrappedResult &result);
