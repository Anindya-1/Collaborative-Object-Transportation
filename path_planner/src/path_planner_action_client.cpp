#include "path_planner/path_planner_action_client.hpp"
...
PathPlanner::PathPlanner() : Node("path_planner"),
    ...
    leader_trajectory_client_= rclcpp_action::create_client<Trajectory2D>(this, "leader_trajectory");    
    follower_trajectory_client_= rclcpp_action::create_client<Trajectory2D>(this, "follower_trajectory");
...
void PathPlanner::Dijkstra() {
    ...
    send_leader_goal(leader_trajectory);
    send_follower_goal(follower_trajectory);
}
...
void PathPlanner::send_leader_goal(std::vector<geometry_msgs::msg::Vector3> trajectory){
    // Wait for the Action Server
    leader_trajectory_client_->wait_for_action_server();

    // Create a goal
    auto goal = Trajectory2D::Goal();
    goal.trajectory = trajectory;

    // Add callbacks
    auto options = rclcpp_action::Client<Trajectory2D>::SendGoalOptions();
    options.result_callback = std::bind(&PathPlanner::goal_result_callback, this, _1);

    // Send the goal
    RCLCPP_INFO(this->get_logger(), "Sending the trajectory");
    leader_trajectory_client_->async_send_goal(goal,options);
}

void PathPlanner::send_follower_goal(std::vector<geometry_msgs::msg::Vector3> trajectory){
    // Wait for the Action Server
    follower_trajectory_client_->wait_for_action_server();
...
void PathPlanner::goal_result_callback(const Trajectory2DGoalHandle::WrappedResult &result){
    bool terminal_state_reached = result.result->reached_terminal_state;
    RCLCPP_INFO(this->get_logger(), "Result: %s", terminal_state_reached ? "true" : "false");
}
