...
class R1ManipControlNode(Node):
    def __init__(self):
        super().__init__('r1_manip_control_node')

        self.declare_parameter("robot_identity", "r1")
    ...
    def create_publishers(self):
        ...
        self.ee_pos_pub = self.create_publisher(Point, f'/{self.robot_name}/ee_position', 10)
    ...
    def read_odometry_callback(self, msg):
        ...
        self.current_base_position[2] = yaw  # correct theta
    ...
    def timer_callback(self):
        ...
        if self.traj_received:
            ...
            self.publish_ee_position_in_world(est_ee_pos)
    ...
    def publish_ee_position_in_world(self, ee_pos_base_frame):
        """
        Publishes the end-effector position in world frame by transforming from base frame.
        :param ee_pos_base_frame: np.array([x, y, z]) in robot base frame
        """
        x_base, y_base, theta_base = self.current_base_position
        x_ee, y_ee, z_ee = ee_pos_base_frame

        # Rotation and translation
        x_world = x_base + math.cos(theta_base) * x_ee - math.sin(theta_base) * y_ee
        y_world = y_base + math.sin(theta_base) * x_ee + math.cos(theta_base) * y_ee
        z_world = z_ee + self.base_height
