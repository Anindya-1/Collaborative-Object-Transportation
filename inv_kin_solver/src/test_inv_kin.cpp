#include "inv_kin_solver/test_inv_kin.hpp"

namespace test_inv_kin {

TestInvKin::TestInvKin() : Node("test_inv_kin_node")
    ,delta_t(0.2), traj_received_(false)
{   
    this->declare_parameter("x_ee", 0.38);
    this->declare_parameter("y_ee", 0.0);
    this->declare_parameter("z_ee", 0.077);
    ...
    odom_subscription_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "/r1/odom", 1, std::bind(&TestInvKin::readOdometryCallback, this, std::placeholders::_1));

    cmd_vel_subscription_ = this->create_subscription<geometry_msgs::msg::Twist>(
        "/r1/cmd_vel", 1, std::bind(&TestInvKin::readCmdVelCallback, this, std::placeholders::_1));

    traj_pub_ = this->create_publisher<trajectory_msgs::msg::JointTrajectory>(
        "/r1/arm_controller/joint_trajectory", 10);

    timer_ = this->create_wall_timer(
        std::chrono::milliseconds(static_cast<int>(delta_t*1000)),
        std::bind(&TestInvKin::timerCallback, this));
        
    RCLCPP_INFO(this->get_logger(), "Node %s has started", this->get_logger().get_name());
}

void TestInvKin::timerCallback(){
...
void TestInvKin::readOdometryCallback(const nav_msgs::msg::Odometry::SharedPtr odom){
...
void TestInvKin::readCmdVelCallback(const geometry_msgs::msg::Twist::SharedPtr cmd_vel){
...
tf2::Vector3 TestInvKin::base_for_kin_solver(tf2::Vector3 current_pos, geometry_msgs::msg::Twist cmd_vel){
    // Parameters
    double dt = delta_t;
...
geometry_msgs::msg::Point TestInvKin::closest_point_on_trajectory_to_vertical_line(
...
void TestInvKin::send_joint_command(const alglib::real_1d_array& joint_angles, double duration_sec){
    ...
    for(int i{0}; i < 4; i++){
        if(i == 1){
            joint_positions.push_back(joint_angles[i]+1.38);    
        }
        else if(i == 2){
            joint_positions.push_back(joint_angles[i]-1.38);
        }
...
int main(int argc, char **argv)
{
    ...
    std::shared_ptr<test_inv_kin::TestInvKin> node = std::make_shared<test_inv_kin::TestInvKin>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
